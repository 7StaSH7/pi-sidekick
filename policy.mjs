...
export const DEFAULT_FUSION_CONFIG = Object.freeze({ sidekick: DEFAULT_SIDEKICK, timeoutMinutes: DEFAULT_TIMEOUT_MINUTES });
export const TOOL = 'fusion_sidekick';
export const STATE = 'pi-fusion-state-v1';
export const STATS = 'pi-fusion-stats-v1';
export const WORKER_ENV = 'PI_FUSION_WORKER';
export const WORKER_CONFIG_ENV = 'PI_FUSION_SIDEKICK_CONFIG';
...
export function validateSidekickSelection(value) {
  if (!plainObject(value) || !exactKeys(value, ['provider', 'id', 'thinking'])) throw new Error('Invalid Fusion sidekick selection.');
  if (!SIDEKICK_PROVIDERS.includes(value.provider)) throw new Error(`Unsupported Fusion sidekick provider: ${String(value.provider)}. Use openai-codex or openai.`);
  if (!modelId(value.id)) throw new Error('Invalid Fusion sidekick model id.');
  if (!THINKING_LEVELS.includes(value.thinking)) throw new Error(`Invalid Fusion sidekick reasoning level: ${String(value.thinking)}.`);
...
export function validateTimeoutMinutes(value) {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 1 || value > MAX_TIMEOUT_MINUTES) {
    throw new Error(`Invalid Fusion timeoutMinutes. Expected an integer from 1 to ${MAX_TIMEOUT_MINUTES}.`);
  }
  return value;
}

export function validateFusionConfig(value) {
  if (!plainObject(value) || !Object.keys(value).every(key => ['sidekick', 'timeoutMinutes', 'animation'].includes(key)) || !Object.hasOwn(value, 'sidekick')) {
    throw new Error('Invalid Fusion config. Expected { sidekick: { provider, id, thinking }, timeoutMinutes }.');
  }
  if (Object.hasOwn(value, 'animation') && typeof value.animation !== 'boolean') {
    throw new Error('Invalid legacy Fusion animation field.');
  }
  return {
...
export function defaultFusionConfig() {
...
export function sidekickPrompt(selection) {
  return `
## Fusion sidekick: ${displaySidekick(selection)}
...
## Fusion mode: configurable sidekick
You remain the user-facing lead. Own the plan, interpretation of ambiguity, and final review.
Use fusion_sidekick for bounded implementation, tests, and well-scoped exploration. It is one persistent configured sidekick session, not a fresh agent per call.
...
For corrections, call fusion_sidekick again with precise feedback; it remembers its own work. If it is blocked or fails, revise the brief or take over using your normal tools. Never replace models or providers to hide a failure.
Call fusion_sidekick alone in its tool batch. Do not run another worker or edit files concurrently with it. Only this sidekick performs delegated work; do not nest other delegation tools for Fusion tasks.
...
export function assertModel(model, expected, thinking) {
  if (model?.provider !== expected.provider || model?.id !== expected.id) {
    throw new Error(`Fusion sidekick requires ${expected.provider}/${expected.id}; no model fallback is allowed.`);
  }
  if (thinking !== undefined && thinking !== expected.thinking) {
    throw new Error(`Fusion sidekick requires ${expected.id} reasoning=${expected.thinking}, got ${thinking}.`);
  }
}
...
export function parseLaunchSidekick(value) {
  try {
    ...
    throw new Error(`Invalid launch-pinned Fusion sidekick config: ${String(error)}`);
  }
}
...
export function restoreState(branch) {
  const entry = branch.findLast(e => e.type === 'custom' && e.customType === STATE);
  if (!entry) return undefined;
  const data = entry.data;
  if (!data || typeof data.enabled !== 'boolean') throw new Error('Invalid Fusion session state.');
  if (data.checkpoint && (typeof data.checkpoint.file !== 'string' ||
    ...
    throw new Error('Invalid Fusion sidekick checkpoint.');
  }
...
export function hasSidekickSibling(branch, name) {
  ...
  return entry?.message.content?.some(c => c.type === 'toolCall' && c.name === TOOL) ?? false;
}
